...
**Decision:** Delete `OpenClawNormativeLayout`. Use `NormativeChannelLayout` from `io.casehub.api.spi.mesh` in `casehub-engine-api`. The blocking provider holds a `private final CaseChannelLayout layout = new NormativeChannelLayout()` field and calls `layout.channelsFor(caseId, null)` at runtime. Unknown purposes now throw `IllegalArgumentException` (previously silently created unconstrained channels). The local protocol was deleted and promoted to the garden as a platform-scope protocol. (The reactive provider was deleted in openclaw#74 — engine virtual thread migration removed all reactive SPIs.)
...
| ~~Reactive SPI variants not built~~ | ~~`ReactiveWorkerProvisioner`, `ReactiveCaseChannelProvider` planned but deferred~~ | casehubio/openclaw#12 **CLOSED** | **Fixed 2026-06-09** — implemented and gated on `casehub.qhorus.reactive.enabled`. **Deleted 2026-07-25** (openclaw#74) — engine virtual thread migration removed all reactive SPIs; blocking variants are now the only implementations. |
