...
**Not owned here:** ActionRiskClassifier SPI in casehub-engine-api (shipped via casehubio/engine#402). Oversight channel uses `deniedTypes=EVENT` per PLATFORM.md (claudony#142 resolved in #25).
...
| Strategy | Completion signaling is via MCP tool calls (`casehub_done`, `casehub_reject`, etc.) — the tool call IS the typed signal; text output via `deliver:webhook` is archived as STATUS (openclaw#28, tool-call-first). |
...
| L2 CaseHub SPI | `casehub/` | WorkerProvisioner, ChannelBackend, OversightGate (fulfill path), CaseChannelProvider |
...
C4Container
  ...
  Container_Boundary(b2, "Layer: L2 — casehub/") {
    ...
    Container(gate, "OversightGateService", "@ApplicationScoped", "Archives webhook text output as STATUS; fulfill() handles oversight gate responses (gate entry rewired at CommitmentTools.done() in openclaw#30)")
  ...
  Rel(delivery, gate, "evaluate(channelId, payload) — archives as STATUS")
...
### Scenario 1 — COMMAND → OpenClaw → DONE round trip (tool-call-first, openclaw#28)

CaseHub engine dispatches COMMAND (with correlationId = commitmentId) on the case work channel. `OpenClawChannelBackend` receives the message via Qhorus fanOut. It appends a fully-resolved commitment context block to the message (agentId and commitmentId as concrete values) and calls `OpenClawHookClient.invoke(agentId, augmentedMessage)`, posting to `POST /hooks/agent`. OpenClaw executes the agent turn. The TypeScript plugin's `before_prompt_build` hook injects ChannelContextWindow content before the turn starts.

During the agent turn, the agent calls `casehub_done(agentId, commitmentId, outcome)` via MCP → `CommitmentTools.done()` → dispatches DONE to the Qhorus work channel with `inReplyTo=COMMAND`, `correlationId=commitmentId` → commitment FULFILLED.

On turn completion, OpenClaw POSTs the agent's text output to `POST /openclaw/delivery/channel/{channelId}`. `OpenClawDeliveryResource` calls `OversightGateService.evaluate()` which archives the text as a non-resolving STATUS message (no correlationId — does not trigger `commitmentService.acknowledge()` in Qhorus). The channel record contains both the typed DONE (completion signal) and the archival STATUS (agent text output).
...
- **STATUS dispatched instead of DONE (Phase 1 workaround)**
  ...
  - **Fix:** Resolved in casehubio/openclaw#16 (2026-06-04) — `OversightGateService.evaluate()` now uses `commitmentStore.findOpenByObligor(agentId, workChannelId)` to recover the correlationId from the durable store and dispatch DONE with correct `inReplyTo`. Restart-resilient; no in-memory state.
...
|---|---|---|---|
| OpenClaw-CaseHub Integration | A Claude Code agent session receives CaseHub skill injection, executes tools with full commitment tracking, and returns accountable DONE/DECLINE outcomes that CaseHub can audit and trust-weight. | 8 (C1–C8) | ✅ Complete |
...
**Delivered:** 2026-05-28 | **Issues:** casehubio/openclaw#3 | **Blog:** `blog/2026-05-28-mdp01-channel-context-window.md`
...
**Journey:** OpenClaw-CaseHub Integration | **Sequence:** 8 of 8 | **Status:** ✅ complete
**Delivered:** 2026-06-05 | **Issues:** casehubio/openclaw#8, casehubio/openclaw#10 | **Blog:** `blog/2026-06-05-mdp01-c8-speech-act-classification.md`

**What this delivers**
Three-tier speech act detection pipeline: JSON envelope (`{"type":"DONE","content":"..."}`) → bracket prefix (`[STATUS] message`) → STATUS fallback. `SpeechActDetection` public utility encapsulates detection; `DefaultSpeechActClassifier` applies STATUS fallback (not DONE — false completion is worse than stuck commitment). `SpeechActClassifier` returns `SpeechActResult(type, content, tier)`. `OversightGateService` uses stripped content for dispatch and oversight prompt; raw output preserved for Qhorus COMMAND audit record. `casehub-global` SKILL.md gains case step response protocol. ADR-0003 documents the STATUS fallback decision.

**Accountability gaps closed**
- ✅ Agent output routed as DONE regardless of speech act type: closed by `SpeechActDetection` + `DefaultSpeechActClassifier` Phase 2/3.
...
`ActionRiskClassifier` migrated to casehubio/engine#402 SPI (closed).
...
3. Create an `@ApplicationScoped` client bean that owns an in-memory session registry (`ConcurrentHashMap<String, Session>`). Key by the runtime's agent identifier (not the CaseHub workerId — the workerId is not stable across provisioning calls; see casehubio/engine#402).
...
**Architectural patterns:** Observer (ChannelContextWindowObserver), Registry (OpenClawAgentRegistry). Tool-call-first: MCP tools own completion signaling; OversightGateService.evaluate() archives webhook text as STATUS (openclaw#28).
...
**Improvement refs:** casehubio/engine#402 (ActionRiskClassifier SPI — Phase 2 gate wiring via openclaw#30), casehubio/openclaw#27 (NliSpeechActClassifier — CLOSED, superseded by openclaw#28 tool-call-first architecture)
...
Not closed here: REST endpoints for delivery webhooks (L3), ActionRiskClassifier SPI in engine-api (casehubio/engine#402). *(#16 DONE dispatch resolved in Epic 8 Phase 2, 2026-06-04.)*
...
- `casehub/src/main/java/.../casehub/OversightGateService.java` — `@ApplicationScoped`; `evaluate(channelId, payload)` archives webhook text as non-resolving STATUS (openclaw#28); `fulfill(gateId, payload)` looks up Commitment by correlationId for Phase 2 gate fulfillment
...
**`@Alternative @Priority(1)` for SPI overrides.** Override implementations of casehub-openclaw SPIs must use `@Alternative @Priority(1)` (not `@DefaultBean`) — defaults do not use `@DefaultBean`, so any `@ApplicationScoped` alternative would cause CDI ambiguity without the `@Alternative` marker. (Note: `ActionRiskClassifier` and `SpeechActClassifier` were deleted in openclaw#28 — tool-call-first completion; Phase 2 risk classification used casehub-engine-api engine#402 SPI (openclaw#30, engine#402 — both closed).)

#### Architectural decisions

**Why `ActionRiskClassifier` was local rather than in casehub-engine-api:** the platform-wide SPI (casehubio/engine#402) was not shipped when this layer was built. A local interface with the same contract allowed the gate to be fully wired and tested with a Phase 1 always-Autonomous default. engine-api shipped the SPI (engine#402 closed); the migration was a pure import swap — the contract was identical by design.
...
Not closed here: *(all known gaps closed — #15 atomicity and #16 DONE dispatch resolved in Epic 7/8 cleanup.)*
...
- `app/src/main/java/.../app/example/ExampleController.java` — `@ApplicationScoped @Blocking`; `POST /example/{exampleId}/start`; demo scenario orchestrator; inert when `casehub.example.enabled=false` (default); closes #35
- `app/src/main/java/.../app/example/ExampleSetup.java` — `@ApplicationScoped @Transactional`; channel creation, agent registration, COMMAND dispatch for demo scenarios
- `app/src/main/java/.../app/example/ExamplePoller.java` — `@ApplicationScoped @Transactional`; JPA polling delegate for ExampleController (null = not yet visible, keep polling; isTerminal() = stop)
- `app/src/main/java/.../app/example/DemoGateClassifier.java` — `@ApplicationScoped @RiskClassifier`; gates on agentId match from `casehub.example.gate.agentid` (Optional<String>); inert when absent
...
**Navigation:** `git log --grep="#7" --oneline`
**Blog:** `blog/2026-05-31-mdp01-openclaw-skill-pack.md`
**Improvement refs:** Phase 2 skills (casehub-reject, casehub-block, casehub-delegate) delivered in openclaw#23
**Completed:** 2026-05-31
...
Not closed here: Phase 2 multi-step skills (broadcast, vote, handoff). NLI speech act classification (openclaw#27) superseded by tool-call-first architecture (openclaw#28).
...
### ADR-0003 — Channel layout topology moved to engine-api; unknown purposes throw

**Date:** 2026-06-22
**Context:** openclaw#38 (parent#93). `OpenClawNormativeLayout` in `casehub/` was a private duplicate of `NormativeChannelLayout` in claudony. Both defined the same 3-channel topology (work/observe/oversight) with identical constraints. The local openclaw protocol `normative-layout-single-source.md` mandated `OpenClawNormativeLayout` as the source of truth for openclaw only.
**Decision:** Delete `OpenClawNormativeLayout`. Use `NormativeChannelLayout` from `io.casehub.api.spi.mesh` in `casehub-engine-api`. Both providers now hold a `private final CaseChannelLayout layout = new NormativeChannelLayout()` field and call `layout.channelsFor(caseId, null)` at runtime. The reactive provider's `openOrCreate` signature changed from `(UUID, String)` to `(UUID, CaseChannelLayout.ChannelSpec)` — the spec is passed directly, eliminating the second `LAYOUT.get(purpose)` lookup. Unknown purposes now throw `IllegalArgumentException` (previously silently created unconstrained channels). The local protocol was deleted and promoted to the garden as a platform-scope protocol.
**Consequences:** openclaw channel topology is now governed by engine-api test guards (`NormativeChannelLayoutTest`). Any change to the normative layout requires an engine-api PR. openclaw#40 (closed) made layout config-driven via a `channelLayout` property (matching claudony's pattern) so operators can switch without recompiling.
**Source:** `casehub/garden: docs/protocols/casehub/normative-channel-layout-single-source.md`
...
### Auth annotation strategy for integration tier REST resources (openclaw#41)

**Date:** 2026-06-23
**Context:** casehub-openclaw's REST surface is predominantly system-to-system — OpenClaw webhook callbacks, Python SDK calls, TypeScript plugin calls. None of these callers carry a casehub OIDC token. This is a structural reality of the integration tier, not a configuration gap. Activating `casehub-platform-oidc` raised the question of which endpoints get `@RolesAllowed` vs `@PermitAll`.
**Decision:** `@PermitAll` on three system-to-system resources (delivery×2, channel-context); `@RolesAllowed(OpenClawGroups.PLUGIN)` on `PluginCommitResource` (authenticated by `PluginTokenBridgeMechanism` via pre-shared bearer token — openclaw#42); `@RolesAllowed(OpenClawGroups.ADMIN)` on `ExampleController`; MCP endpoint secured by HTTP auth policy `authenticated` (openclaw#43). `@PermitAll` is preferred over leaving endpoints unannotated — it makes the deliberate absence of auth auditable. CDI exclusions for `MockCurrentPrincipal` and `QhorusInboundCurrentPrincipal` removed (openclaw#48) — `OidcCurrentPrincipal @Alternative @Priority(100)` (platform#111) displaces both via standard CDI resolution. `OpenClawCurrentPrincipal @Alternative @Priority(150)` handles bridge-authenticated identities (non-OIDC SecurityIdentity → default tenancyId).
**Consequences:** Plugin endpoint auth (openclaw#42) and MCP endpoint auth (openclaw#43) shipped. Delivery endpoints remain `@PermitAll` — OpenClaw webhooks carry no OIDC token. OIDC client-credentials migration for plugin (openclaw#52) deferred pending upstream OpenClaw support. Platform#121 shipped — `OidcCurrentPrincipal` now handles non-OIDC SecurityIdentity types natively.
**Source:** `docs/superpowers/specs/2026-06-23-oidc-wiring-design.md`, `docs/protocols/casehub/oidc-cdi-qhorus-exclusion.md`.

### PlannedAction dependency resolution — coding against engine local HEAD (openclaw#46)

**Date:** 2026-06-23
**Context:** The published `casehub-engine-api` SNAPSHOT was built from an interim state that removed `PlannedAction` from `io.casehub.api.spi` (anticipating spec #543 — worker primitives migration). Local engine `main` still has `io.casehub.api.spi.PlannedAction` as a 5-field record `(workerId, caseId, description, actionType, context)` and `ActionRiskClassifier.classify(PlannedAction)` as a one-arg method. GitHub Packages 401 auth prevented downloading the published SNAPSHOT.
**Decision:** Code against local engine HEAD. `OversightGateService` constructs `PlannedAction(agentId, caseId, outcome, "COMPLETION", Map.of())` inline and passes it to classifiers. No `casehub-worker-api` dependency; no `ClassificationContext`. Filed engine#565 to re-publish SNAPSHOT from local main. engine#543 shipped (ClassificationContext, worker-api migration); openclaw adaptation was completed in openclaw#46.
**Consequences:** openclaw#46 closed. The `ClassificationContext` separation (tracking workerId/caseId separately from action description) remains a future concern — it will improve classifier context quality when implemented.
...
|---|---|---|---|
| ~~`ActionRiskClassifier` SPI not in engine-api~~ | ~~Local placeholder; migration to `casehub-engine-api` import swap depends on casehubio/engine#402 shipping~~ | casehubio/engine#402 **CLOSED** | **Resolved** — SPI shipped in engine-api; import swap completed |
...
| ~~Epic 3 issue (#3) never closed~~ | ~~`ChannelContextWindow` code is complete; the GitHub epic issue was left open after the Flyway migration was dropped from scope~~ | casehubio/openclaw#3 **CLOSED** | **Resolved** — issue closed |

### Technical Debt

| Item | Description | Issue | Priority |
|---|---|---|---|
| ~~STATUS instead of DONE~~ | ~~`OpenClawDeliveryResource` dispatches STATUS (not DONE) because `inReplyTo` Long is unavailable at delivery time~~ | casehubio/openclaw#16 **CLOSED** | **Fixed 2026-06-04** — `evaluate()` uses `findOpenByObligor()` to recover correlationId and dispatch DONE with correct `inReplyTo` |
| ~~`fulfill()` atomicity gap~~ | ~~`OversightGateService.fulfill()` dispatches RESPONSE and DONE in two sequential calls~~ | casehubio/openclaw#15 **CLOSED** | **Fixed 2026-06-03** — `OversightGateDispatcher @ApplicationScoped` wraps both dispatches atomically |
| ~~Oversight channel `allowedTypes`~~ | ~~Set to null (all types allowed); final decision deferred to casehubio/claudony#142~~ | claudony#142 **CLOSED** | **Resolved** — oversight channel uses `deniedTypes=EVENT` per PLATFORM.md (openclaw#25) |
| ~~Webhook payload `@JsonAlias` unverified~~ | ~~Field names in `OpenClawDeliveryPayload` may differ from live OpenClaw API; assumed camelCase, unconfirmed~~ | casehubio/openclaw#11 **CLOSED** | **Resolved** |
| ~~`channelId` not stored on Commitment~~ | ~~`OversightGateService` must reconstruct `channelId` from Qhorus queries after restart; storing it would simplify `fulfill()`~~ | casehubio/openclaw#20 **CLOSED** | **Resolved** |
| ~~Reactive SPI variants not built~~ | ~~`ReactiveWorkerProvisioner`, `ReactiveCaseChannelProvider` planned but deferred~~ | casehubio/openclaw#12 **CLOSED** | **Fixed 2026-06-09** — `ReactiveOpenClawWorkerProvisioner` and `ReactiveOpenClawCaseChannelProvider` implemented; `OpenClawCaseChannelProvider.openChannel()` initChannel() bug fixed; gated on `casehub.qhorus.reactive.enabled` |
| ~~Epic 6 code review findings~~ | ~~Minor code quality issues from review; not correctness defects~~ | casehubio/openclaw#17 **CLOSED** | **Resolved** |
| ~~Deep-dive doc stale for Epic 7 MCP layer~~ | ~~`../parent/docs/repos/casehub-openclaw.md` not updated for Epic 7 MCP endpoint~~ | casehubio/openclaw#19 **CLOSED** | **Resolved 2026-06-09** — doc already updated in parent repo; verified Layer 0 section and PLATFORM.md Capability Ownership entry both present |
...
| STATUS | Qhorus message type acknowledging an obligation without resolving it |
