...
| [normative-layout-single-source.md](normative-layout-single-source.md) | All channel layout changes must go through NormativeChannelLayout (from casehub-engine-api) — never in individual providers | casehub/ — any code creating or configuring Qhorus channels per case |
| [oidc-cdi-qhorus-exclusion.md](oidc-cdi-qhorus-exclusion.md) | Exclude QhorusInboundCurrentPrincipal from CDI when casehub-platform-oidc is on the classpath | app/pom.xml + application.properties — any commit adding casehub-platform-oidc |
